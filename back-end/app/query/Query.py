...
import json
from . import queryProcess
from . import parseQuery
# from queryProcess import processData
# from parseQuery import parse_query


sparql = SPARQLWrapper("http://124.222.44.207:3030/medicals/") #这里要改现在的项目
...
def query(query_string):
    key_words = parseQuery.parse_query(query_string)
    # key_words = '麝香'
    ...
    if result == False and state == 3 and key_words[-1] not in pred_list:
        state = 2
    result = queryProcess.processData(key_words, result, state)
    return result
