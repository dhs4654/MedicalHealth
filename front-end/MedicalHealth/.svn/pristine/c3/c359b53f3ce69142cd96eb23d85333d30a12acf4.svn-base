// packages/avm-ui/src/components/check-list/check-list.less
var check_list_default = ".adm-check-list-item-extra {\n  font-size: 16px;\n  color: #9AC200;\n}\n.adm-check-list-item-readonly {\n  cursor: unset;\n}\n.adm-check-list-item-active {\n  color: #9AC200;\n}\n";
...
var CheckListItem = class extends Component {
  render = (props2) => {
    const active = props2.checked ? props2.checked.includes(props2.value) : false;
    const readOnly = props2.readOnly;
    const activeIcon = props2.activeIcon;
    const extra = <div className={`${classPrefix4}-extra`}>{active ? activeIcon ?? <Icon type="CheckCircleFill" color={active ? "#9AC200" : "#666"} /> : ""}</div>;
    return <ListItem title={props2.title} className={classNames({
      [`${classPrefix4}-readonly`]: readOnly
    })} active={active} description={props2.description} prefix={props2.prefix} onHandleClick={() => {
      if (readOnly)
        return;
      if (active) {
        props2.uncheck(props2.value);
      } else {
        props2.check(props2.value);
      }
      props2.handleClick?.(props2.value, active);
    }} arrow={false} clickable={!readOnly} extra={extra} disabled={props2.disabled}>{props2.label}</ListItem>;
  };
};
...
var CheckList = class extends Component {
  ...
  check = (val, props2) => {
    ...
    props2.handleChange && props2.handleChange(this.data.checked);
  };
  uncheck = (val, props2) => {
  ...
  render = (props2) => {
    ...
    const childEle = list.map((item) => {
      const disabled = item.disabled ?? props2.disabled;
      const readOnly = item.readOnly ?? props2.readOnly;
      return <CheckListItem {...item} disabled={disabled} readOnly={readOnly} checked={this.data.checked} activeIcon={props2.activeIcon} check={() => this.check(item.value, props2)} uncheck={() => this.uncheck(item.value, props2)} />;
    });
    return <List_default mode={props2.mode}>{childEle}</List_default>;
  };
  css = () => check_list_default;
};

// packages/avm-ui/src/components/check-list/index.ts
var check_list_default2 = CheckList;
avm.define("avm-check-list", CheckList);
export {
  check_list_default2 as default
};
